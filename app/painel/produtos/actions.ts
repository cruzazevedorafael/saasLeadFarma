...
export async function createProduto(input: ProdutoInput) {
  ...
  const { data: prod, error } = await db.from('products').insert({
    ...
    price_wholesale: data.hasWholesale ? data.priceWholesale : data.priceRetail,
    price_retail: data.priceRetail, has_wholesale: data.hasWholesale,
    weight_grams: data.weightGrams,
    counts_for_wholesale: data.hasWholesale && data.countsForWholesale, active: data.active,
