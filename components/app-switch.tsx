...
export function AppSwitch({ href, variant }: { href: string; variant: 'catalogo' | 'painel' }) {
  ...
  // Fica na ESQUERDA, acima do "Instalar app" (bottom-left), pra NÃO colidir com o
  // carrinho (bottom-right) no catálogo.
  return (
    <Link
      href={href}
      className="fixed bottom-[4.75rem] left-4 z-40 flex items-center gap-2 rounded-full bg-foreground px-4 py-2.5 text-sm font-semibold text-background shadow-lg transition hover:brightness-110 active:scale-95"
    >
